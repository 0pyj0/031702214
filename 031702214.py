...
def hprovince():
    ...
    if province ==None:#直辖市
        ...
        province=province.group(0)
    if province in ('北京', '上海', '天津', '重庆'):
        province = province
        #mlist=re.sub(province,province + "市",mlist,1)
        mlist = mlist.replace(province, province + "市", 1)
    elif province in ('北京市', '上海市', '天津市', '重庆市'):
        province = province[0:2]
    elif province == "广西":
        mlist=re.sub(province,'',mlist,1)
        #mlist = mlist.replace(province, "", 1)
...
def hcity():
    ...
    if city==None:
        ...
        city=city.group(0)
    if city != "":
        mlist=re.sub(city,'',mlist,1)
        #mlist = mlist.replace(city, "", 1)
...
def hdistrict():
    ...
    if district != "":
        L = len(district)
        if district[-1] == mlist[L-1]:
            mlist=re.sub(district,'',mlist,1)
            #mlist = mlist.replace(district, "", 1)
        else:
            mlist=re.sub(district[:-1],'',mlist,1)
            #mlist = mlist.replace(district[:-1], "", 1)
    dis=district
...
def htown():
    global mlist
    global tow
    town = re.search("(.*?[镇乡])|(.*?街道)",mlist)
    if town ==None:
        town=""
    else:
        town=town.group(0)
    mlist=re.sub(town,'',mlist,1)
    #mlist= mlist.replace(town, "", 1)
    tow=town
